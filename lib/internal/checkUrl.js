...
function checkUrl(link, baseUrl, cache, options, retry)
{
	...
	// Only add authorization header for links with same base url
	if (options.authorization !== undefined && link.url.resolved.includes(link.base.resolved))
	...
	var request = bhttp.request(link.url.resolved,  // TODO :: https://github.com/joepie91/node-bhttp/issues/3
	{
		...
		responseTimeout: 5000 // Add response timeout after 5s
	})

...
cli.prototype.input = function(args, showArgs)
{
	...
	else if (options.input != null)
	{
		...
		// Only set these if they're defined (to avoid overriding defaults with undefined)
		if (options.maxSockets !== undefined) {
			checkerOpts.maxSockets = options.maxSockets;
		}
		if (options.maxSocketsPerHost !== undefined) {
			checkerOpts.maxSocketsPerHost = options.maxSocketsPerHost;
		}
...
function run(url, checkerOptions, logOptions)
{
	...
	var handlers = {
		...
		link: function(result)
		{
			...
			{
				cachedLinks++;
				// Don't show non-broken cached links unless verbose mode is on
				if (logOptions.excludeCachedLinks === true)
				{
					shouldShow = false;
				}
			}
		...
		page: function(error, pageUrl)
		{
			if (error != null)
			{
				// For non-HTML content type errors, display as a warning instead of an error
				if (error.code === "BLC_NON_HTML" || error.isWarning)
				{
					log(chalk.yellow("Warning: ") + chalk.gray(error.message) + chalk.gray(" - skipping " + pageUrl));
				}
				else
				{
